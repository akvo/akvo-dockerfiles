...
slack_txt=\$(git log --oneline $OLDER_GIT_VERSION..$NEWEST_GIT_VERSION | grep -v "Merge pull request" | grep -v "Merge branch" | cut -f 2- -d\  | sed 's/\[#\([0-9]*\)\]/<https:\/\/github.com\/akvo\/akvo-flow-services\/issues\/\1|[#\1]>/' | tr \" \')
...
\$CMD \
    chat send \
    --channel='#flumen-dev' \
    --pretext="$MSG. <https://github.com/akvo/akvo-flow-services/compare/$OLDER_GIT_VERSION..$NEWEST_GIT_VERSION|Full diff>." \
    --color $COLOR \
